...
class PersonDetect:
    ...
    def load_model(self):
        ...
        '''
        ie_core = IECore()
        self.net = ie_core.load_network(network=self.model, device_name=self.device, num_requests=1)  
    ...
    def preprocess_outputs(self, outputs):
        '''
        Extract valid boxes from inference results
        '''
        valid_boxes = []
        # squeeze the dimensions since model returns in the form of multi dimension array
        outputs = np.squeeze(outputs)
        # each output is in the form of image_id, label, threshold, xmin, ymin, xmax, ymax
    ...
    def draw_outputs(self, coords, image):
        ...
        '''
        # since we have resized image need to restore the coordinates to same ratio
        ...
        return valid_boxes, image
        
    def predict(self, image):
        '''
        Returns predictions for the feeded image
        '''
        input_img = self.preprocess_input(image)
        infer_out  = self.net.infer({self.input_name: input_img})
        outputs = infer_out[self.output_name]
        boxes = self.preprocess_outputs(outputs)
        valid_boxes, output_img = self.draw_outputs(boxes, image)
        return valid_boxes, output_img
